...
fn run_fragmented_criterion_bench(query: Query<&HeavyTransform>, targets: Res<BenchmarkTargets>) {
    ...
    let mut group = c.benchmark_group("ecs_scrambled_heavy_lookup");
    let target_count = targets.entities.len();

    group.bench_with_input("avenix_safe_lookup_heavy", &target_count, |b, _| {
        b.iter(|| {
    ...
    group.bench_with_input("avenix_unchecked_lookup_heavy", &target_count, |b, _| {
        b.iter(|| {
            for entity in &targets.entities {
                unsafe {
